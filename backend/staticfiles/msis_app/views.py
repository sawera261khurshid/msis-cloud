...
class MachineView(APIView):
    ...
    def get(self, request):
        ...
        # shift_end = current_time.replace(hour=20, minute=0, second=0, microsecond=0)
        ...
        for key, model_class in model_classes.items():
            model_data = model_class.objects.filter(
                ...
                timestamp__gte=shift_start  # adjust if you want to use `last_reset_time` per machine
            ).values('machine').annotate(
                anomaly_count=Count(Case(When(status='NG', then=1), output_field=IntegerField())),
                detection_count=Count(Case(
        ...
        for machine_id, machine_data in machine_info_map.items():
            ...
            last_reset_time = machine_data.last_reset_time or shift_start
            anomaly_count = 0
            detection_count = 0
            machine_status = "off"
            ...
            if machine_id in data:
                for category_data in data[machine_id].values():
                    ...
                    # TODO check whether status is working correctly
                    logger.debug(f"last_entry_time: {category_data['last_entry_time']}, {time_threshold}, machineID: {machine_id}")
...
class MachineStatsView(APIView):
    ...
    def get(self, request, category, type, machine_id):
        ...
        if type_input == "HOURLY" and date_input.day > datetime.now().day:
        ...
        if type_input == 'DAILY':
            logger.debug(f"date_input: {date_input}")
            date_input = datetime(date_input.year, date_input.month, 1)
            logger.debug(f"date_input 1: {date_input}")
            ...
            logger.debug(f"next_month number: {next_month}")
            next_month = datetime(year_input, next_month, 1)
            logger.debug(f"next_month datetime: {next_month}")
            num_days = (next_month - timedelta(days=1)).day # last day of the month
            logger.debug(f"num of days in month ({date_input}): {num_days}")
            ...
            logger.info(f"{date_input} number of days: {num_days}")
            query = query.filter(
                machine=machine,
                status='NG',
            ...
            end_time = timezone.now() if not date_input else date_input
